...
| **10. 工程化收尾** | 2-3天 | 编写正式测试：单元测试（`tests/unit/`，mock 外部依赖）+ 集成测试（`tests/integration/`，连接真实服务，不进 CI）；GitHub Actions CI（日常 push 触发：并行运行 Ruff + `tests/unit/` + `evals/`，不起外部服务，不构建镜像、不发布 data.tar.gz）；Docker 镜像瘦身（精简构建阶段、剔除无用依赖、利用 uv 缓存层等）；README 撰写、LangSmith 最终 Trace 收集 | `tests/unit/`, `tests/integration/`, `.github/workflows/ci.yml`, `README.md` |
| **v1.0 评估数字采集** | 0.5天 | 发布前跑三轮评估、记录终值——① 确定性指标：`uv run pytest evals/test_deterministic.py -v` → 物理校验通过率 / 旋律保真率 / zone 合规率；② LLM 决策层：`uv run python evals/eval_llm_nodes.py` → Agent 5 指令解析准确率（op 类型 + scope 类别）；③ RAG 检索：`uv run python evals/eval_rag.py` → hit@1 / 正确拒识率。数字写入 README 和 LangSmith Trace，作为简历性能数据的唯一来源。架构改进（ADR-001 P1/P2/P3、chord voicing、zone 硬约束）不报具体百分比差值（无历史基线），以设计决策 + Trace 定性展示替代 | `evals/` |
| **v1.0 正式发布** | 0.5天 | 代码冻结后执行：① 参照 `docs/release-checklist.md` 检查交付模式配置（docker-compose.yml 去开发挂载、Dockerfile 确认模型内置）；② 手动打包 `data.tar.gz`（`data/raw_midi/` + `data/rag/` + `data/curated_fingerstyle/`）；③ 手动打 tag 如 `v1.0` → GitHub Actions CD 在云端构建镜像并推送至 Docker Hub（`yourname/fingerstyle-agent:v1.0`）；④ 在 GitHub Release 页面手动上传 `data.tar.gz` + `docker-compose.yml`（交付版）+ `.env.example` 作为附件；⑤ 撰写 Release Notes（功能简介 + 用户操作步骤）。发布后删除 `docs/release-checklist.md` | `.github/workflows/cd.yml`, `docs/release-checklist.md`, `scripts/pack_data.sh` |
